...
class GraphRelationshipsComputer {
    fun compute(anagrams: List<AnagramMetadata>): Set<Pair<AnagramConnection, AnagramConnection?>> {
    ...
    private fun AnagramMetadata.addAsDescendant(link: AnagramLink): List<AnagramLink> {
        val (matchingChildren, otherChildren) = link.children.partition { child -> this.sortedLetters.isIncludedIn(child.anagram.sortedLetters) }
    ...
    ): Set<Pair<AnagramConnection, AnagramConnection?>> =
            ...
            ?.let { children ->
                children.map {
                    AnagramConnection(link.name, link.ownAnagrams, link.compoundAnagrams) to
                            AnagramConnection(it.name, it.ownAnagrams, it.compoundAnagrams)
                }
                    .toSet()
                    .plus(children.flatMap { computePairsForLink(it, true) }.toSet())
            }
            ?: if (isChild) emptySet() else setOf(
                AnagramConnection(link.name, link.ownAnagrams, link.compoundAnagrams) to null
            )
